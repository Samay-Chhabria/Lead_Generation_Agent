"""Yellow Pages search provider (extension point).

``YellowPagesProvider`` is the clean extension point for the Yellow Pages
backend. It inherits all shared provider behaviour from ``SearchProvider`` and
currently only provides the provider-specific contract: the search is a no-op
that logs a clear message until a real implementation replaces it.

To implement this provider, override the provider-specific parts only:

- ``search()``: drive the browser session through the Yellow Pages workflow
...
class YellowPagesProvider(SearchProvider):
    """Search Yellow Pages for businesses described by a SearchPlan.
