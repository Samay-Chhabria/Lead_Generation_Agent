"""Bing Maps search provider (extension point).

``BingMapsProvider`` is the clean extension point for the Bing Maps backend.
It inherits all shared provider behaviour from ``SearchProvider`` and currently
only provides the provider-specific contract: the search is a no-op that logs a
...
- ``search()``: drive the browser session through the Bing Maps workflow
  (navigation, consent, search, results) and fill ``self._references`` and
...
class BingMapsProvider(SearchProvider):
    """Search Bing Maps for businesses described by a SearchPlan.
