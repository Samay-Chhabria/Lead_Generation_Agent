...
|---|----------------------------|--------|----------------|-------------|----------|
| R1 | Accepts a natural-language prompt | PASS | `app/parser/prompt_parser.py`, `app/main.py` | The user supplies a prompt as a CLI argument or interactively; `PromptParser.parse` converts it into a `SearchPlan` without any manual field entry. | `tests/test_requirement_matrix.py::test_r1_accepts_natural_language_prompt`, `tests/unit/test_prompt_parser.py`, `tests/test_end_to_end.py` |
