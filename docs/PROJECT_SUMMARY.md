...
Behind the scenes the application behaves as an **autonomous agent**: a planner
builds a `TaskPlan` (LLM-first with a deterministic offline fallback), an
executor runs the planned tool steps through a guarded `ToolManager`, and
failure-recovery logic keeps the run alive through navigation errors, consent
dialogs, and unresponsive listings.
...
- **Autonomous agent loop** — `Planner` → `AgentExecutor` → `ToolManager` with
  LLM-first tool selection, guarded execution, and failure recovery.
- **PySide6 desktop GUI** — execution plan panel, live agent status, leads
  table, Excel download, execution timeline, streaming logs, and statistics.
- **Full test coverage**: 483 tests across unit, integration, end-to-end,
  requirement, agent, and performance suites.
...
|------------------|----------------------------------------------------|---------------------------------------------|
| Entry point      | `app/main.py`                                      | Interactive CLI bootstrap                   |
| Application      | `app/application/`                                 | Lifecycle, config loading, logging, exit code |
| Agent            | `app/agent/`                                       | Planner, executor, tool manager, memory, state, console facade |
| Tools            | `app/tools/`                                       | 13-tool registry and wrappers (search, extraction, export, summary, pipeline) |
| LLM              | `app/llm/`                                         | Unified LLM gateway (Free LLM Router) with automatic offline/AI mode switching |
| GUI              | `app/gui/`                                         | PySide6 desktop GUI (pure presentation)     |
| Pipeline         | `app/pipeline/`                                    | Legacy orchestration; exposed as the `pipeline` tool |
...
| Production readiness | `f0b6c04`+ | Repository cleanup, professional README, compliance/summary/checklist docs |
| AI agent (V5) | *this milestone* | Planner/executor/tool manager, LLM tool selection, failure recovery, PySide6 desktop GUI, 483-test suite |
...
- **Requirement** (`tests/test_requirement_matrix.py`,
  ...
  robustness.
- **CLI** (`tests/test_cli.py`) — interactive prompt and process exit codes.
- **Agent / GUI / LLM** (`tests/test_agent_*.py`, `tests/test_planner.py`,
  `tests/test_tool_registry.py`, `tests/test_llm.py`) — planner, executor,
  tool manager, memory, LLM providers, and the GUI controller/log stream.
- **Performance** (`tests/unit/test_performance.py`) — timing budgets and
  resource-cleanup checks at scale.

**483 tests**, covering all 14 requirements (PASS), realistic failure paths,
...
- Source: 100 Python files in `app/`
- Tests: 40 Python files, 483 automated tests, all passing
...
- Richer prompt parsing (multi-location, geo-NER beyond the LLM planner).
